...
text = """table.append(['BRK','inherent'])
...
def commentLines(text, fp=sys.stdout):
    lines = [line.strip() for line in text.split("    ")]
    for index, line in enumerate(lines):
        if index in (0x0a,  0x2a, 0x4a, 0x6a):
            if not 'inherent' in line:
                pass
                #~ raise Exception("can't correct data! in line $%02x\n%s"%(index, line))
            else:
                line = line.replace('inherent', 'accumulator')
                
        if not '# opcode' in line:
            writeln("    %-35s # opcode $%02X"%(line, index))
        else:
            writeln("    %s"%line)
    return len(lines)
...
if __name__ == "__main__":
    ...
    commentLines(text, fp=sys.stdout)
    
    text2 = """00 BRK 7        $00: bytes: 0 cycles: 0 _____=>_____ __ 
